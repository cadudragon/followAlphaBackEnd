    ...
    IOptions<CacheOptions> cacheOptions)
{
    ...
    private readonly TokenMetadataRepository _tokenMetadataRepository = tokenMetadataRepository ?? throw new ArgumentNullException(nameof(tokenMetadataRepository));
    ...
    /// </summary>
    public async Task<List<AlchemyTokenBalance>> GetTokenBalancesAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = await response.Content.ReadFromJsonAsync<AlchemyTokenBalanceResponse>(cancellationToken);

            if (result?.Result?.TokenBalances == null)
            {
                _logger.LogWarning("No token balances found for {Wallet} on {Network}", walletAddress, network);
                return [];
            }

            // Filter out zero balances
            return [.. result.Result.TokenBalances.Where(t => !string.IsNullOrEmpty(t.TokenBalance) && t.TokenBalance != "0x0")];
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching token balances from Alchemy for {Wallet}", walletAddress);
            throw;
        }
    }

    /// <summary>
    /// Get both native balance and ERC20 token balances in a single batched JSON-RPC request.
    /// This method combines eth_getBalance and alchemy_getTokenBalances into one HTTP call,
    /// reducing network round-trips by 50% compared to calling them separately.
    /// Uses JSON-RPC 2.0 batch specification: https://www.jsonrpc.org/specification#batch
    /// </summary>
    /// <param name="walletAddress">Wallet address to query</param>
    /// <param name="network">Network to query</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Tuple containing (native balance as hex string, list of ERC20 token balances)</returns>
    public async Task<(string nativeBalance, List<AlchemyTokenBalance> tokenBalances)> GetAllBalancesAsync(
        string walletAddress,
        BlockchainNetwork network,
        CancellationToken cancellationToken = default)
    {
        ...
        // JSON-RPC 2.0 batch request - both calls to same endpoint in single HTTP request
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync(url, batchRequest, cancellationToken);
            response.EnsureSuccessStatusCode();

            var batchResponse = await response.Content.ReadFromJsonAsync<List<JsonElement>>(cancellationToken);

            if (batchResponse == null || batchResponse.Count < 2)
            {
                _logger.LogError("Invalid batch response from Alchemy for {Wallet} on {Network}", walletAddress, network);
                return ("0x0", []);
            }

            // Parse responses by matching id field
            var nativeBalanceResponse = batchResponse.FirstOrDefault(r =>
                r.TryGetProperty("id", out var id) && id.GetInt32() == 1);
            var tokenBalancesResponse = batchResponse.FirstOrDefault(r =>
                r.TryGetProperty("id", out var id) && id.GetInt32() == 2);

            // Extract native balance using deserialization
            string nativeBalance = "0x0";
            if (nativeBalanceResponse.ValueKind != JsonValueKind.Undefined)
            {
                var nativeResult = JsonSerializer.Deserialize<AlchemyBalanceResponse>(
                    nativeBalanceResponse.GetRawText(),
                    new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                nativeBalance = nativeResult?.Result ?? "0x0";
            }
            else
            {
                _logger.LogWarning("Native balance response not found in batch for {Wallet} on {Network}", walletAddress, network);
            }

            // Extract token balances using deserialization
            List<AlchemyTokenBalance> tokenBalances = [];
            if (tokenBalancesResponse.ValueKind != JsonValueKind.Undefined)
            {
                var tokenResult = JsonSerializer.Deserialize<AlchemyTokenBalanceResponse>(
                    tokenBalancesResponse.GetRawText(),
                    new JsonSerializerOptions { PropertyNameCaseInsensitive = true });

                if (tokenResult?.Result?.TokenBalances != null)
                {
                    // Filter out zero balances
                    tokenBalances = tokenResult.Result.TokenBalances
                        .Where(t => !string.IsNullOrEmpty(t.TokenBalance) && t.TokenBalance != "0x0")
                        .ToList();
                }
                else
                {
                    _logger.LogWarning("No token balances found in batch response for {Wallet} on {Network}", walletAddress, network);
                }
            }
            else
            {
                _logger.LogWarning("Token balances response not found in batch for {Wallet} on {Network}", walletAddress, network);
            }

            _logger.LogInformation(
                "Batch RPC request successful for {Wallet} on {Network}: native={NativeBalance}, ERC20 tokens={TokenCount}",
                walletAddress, network, nativeBalance, tokenBalances.Count);

            return (nativeBalance, tokenBalances);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching batched balances from Alchemy for {Wallet} on {Network}", walletAddress, network);
            throw;
        }
    }

    /// <summary>
    /// Get token balances with metadata and prices across multiple networks.
    /// Uses Alchemy's /assets/tokens/by-address endpoint which returns all data (balances, metadata, prices) in one response.
    /// Automatically batches networks into groups based on configured limit (default: 5 networks per request).
    /// Makes parallel requests for each batch and merges results.
    /// </summary>
    /// <param name="walletAddress">Wallet address to query</param>
    /// <param name="networks">Networks to query (will be batched based on configuration)</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Dictionary mapping network to list of enriched token data</returns>
    public async Task<Dictionary<BlockchainNetwork, List<AlchemyMultiNetworkToken>>> GetMultiNetworkTokenBalancesAsync(
        string walletAddress,
        IEnumerable<BlockchainNetwork>? networks = null,
        CancellationToken cancellationToken = default)
    {
        // Use priority networks from config if none specified
        var networksToQuery = networks?.ToList() ?? _alchemyOptions.GetPriorityNetworks();

        if (networksToQuery.Count == 0)
        {
            //TODO: refactor to throw a error
            _logger.LogWarning("No networks configured for multi-network token balance query");
            return [];
        }

        // Batch networks into groups based on Alchemy's multi-network balances API limit
        var batchSize = _alchemyOptions.BatchLimits.MultiNetworkBalancesMaxNetworks;
        var batches = networksToQuery
            .Select((network, index) => new { network, index })
            .GroupBy(x => x.index / batchSize)
            .Select(g => g.Select(x => x.network).ToList())
            .ToList();

        _logger.LogInformation(
            "Fetching multi-network token balances for {Wallet} across {NetworkCount} networks in {BatchCount} batches of max {BatchSize} networks",
            walletAddress,
            networksToQuery.Count,
            batches.Count,
            batchSize);

        // Make parallel requests for each batch
        var batchTasks = batches.Select(batch => FetchTokensForBatchAsync(walletAddress, batch, cancellationToken));
        var batchResults = await Task.WhenAll(batchTasks);

        // Merge all batch results into a single dictionary
        var mergedResults = new Dictionary<BlockchainNetwork, List<AlchemyMultiNetworkToken>>();
        foreach (var batchResult in batchResults)
        {
            foreach (var kvp in batchResult)
            {
                if (mergedResults.TryGetValue(kvp.Key, out List<AlchemyMultiNetworkToken>? value))
                {
                    value.AddRange(kvp.Value);
                }
                else
                {
                    mergedResults[kvp.Key] = kvp.Value;
                }
            }
        }

        _logger.LogInformation(
            "Multi-network query completed: {TotalTokens} tokens across {NetworksWithBalances} networks",
            mergedResults.Values.Sum(list => list.Count),
            mergedResults.Count);
    ...
    /// <summary>
    /// Fetch tokens for a single batch of networks (max 5).
    /// </summary>
    private async Task<Dictionary<BlockchainNetwork, List<AlchemyMultiNetworkToken>>> FetchTokensForBatchAsync(
        string walletAddress,
        ...
        CancellationToken cancellationToken)
    {
        if (networks.Count > 5)
        {
            throw new ArgumentException("Alchemy API supports maximum 5 networks per request", nameof(networks));
        }

        var url = $"https://api.g.alchemy.com/data/v1/{_apiKey}/assets/tokens/by-address";

        // Map BlockchainNetwork enum to Alchemy network strings
        var alchemyNetworks = networks.Select(n => n.ToAlchemyNetwork()).ToList();
        ...
        {
            _logger.LogDebug(
                "Fetching batch for {Wallet}: {Networks}",
                walletAddress,
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError(
                    "Alchemy multi-network API returned {StatusCode} for batch: {Error}. Networks: {Networks}",
                    response.StatusCode,
                    errorBody,
                    string.Join(", ", alchemyNetworks));
                response.EnsureSuccessStatusCode();
            }

            var result = await response.Content.ReadFromJsonAsync<AlchemyMultiNetworkAssetsResponse>(cancellationToken);

            if (result?.Data?.Tokens == null)
            {
                _logger.LogDebug("No tokens found in batch response for {Wallet}", walletAddress);
                return new Dictionary<BlockchainNetwork, List<AlchemyMultiNetworkToken>>();
            }

            // Group tokens by network and filter out zero balances
            var tokensByNetwork = result.Data.Tokens
                .Where(t => !string.IsNullOrEmpty(t.TokenBalance) && t.TokenBalance != "0" && t.TokenBalance != "0x0")
                .GroupBy(t => BlockchainNetworkExtensions.FromAlchemyNetwork(t.Network ?? "eth-mainnet"))
                .ToDictionary(
                    g => g.Key,
                    g => g.ToList()
                );
            ...
            _logger.LogDebug(
                "Batch returned {TotalTokens} tokens across {NetworksWithBalances} networks",
                result.Data.Tokens.Count,
                tokensByNetwork.Count);

            return tokensByNetwork;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching batch for {Wallet} - Networks: {Networks}",
                walletAddress,
                string.Join(", ", networks));
            throw;
        }
    ...
    /// Checks TokenMetadata cache/DB first, then fetches from Alchemy if not found.
    /// Saves to TokenMetadata after fetching for future use.
    /// </summary>
    public async Task<AlchemyTokenMetadata?> GetTokenMetadataAsync(
        string contractAddress,
        BlockchainNetwork network,
        CancellationToken cancellationToken = default)
    {
        // Check TokenMetadata cache/DB first
        var cachedMetadata = await _tokenMetadataRepository.GetByAddressAsync(
            contractAddress,
            network,
            cancellationToken);
        ...
        {
            _logger.LogDebug(
                "Token metadata found in cache/DB for {Symbol} ({Address}) on {Network}",
                cachedMetadata.Symbol,
                contractAddress,
                network);

            // Convert TokenMetadata to AlchemyTokenMetadata
            return new AlchemyTokenMetadata
            {
                Symbol = cachedMetadata.Symbol,
                Name = cachedMetadata.Name,
                Decimals = cachedMetadata.Decimals,
                Logo = cachedMetadata.LogoUrl
            };
        }

        // Not in cache - fetch from Alchemy
        _logger.LogInformation(
            "Token metadata NOT in cache for {Address} on {Network}, fetching from Alchemy",
            contractAddress,
            network);
        ...
        try
        {
            ...
            var result = await response.Content.ReadFromJsonAsync<AlchemyTokenMetadataResponse>(cancellationToken);
            var alchemyMetadata = result?.Result;

            // Save to TokenMetadata for future use (even if verification fails later)
            if (alchemyMetadata != null &&
                !string.IsNullOrEmpty(alchemyMetadata.Symbol) &&
                !string.IsNullOrEmpty(alchemyMetadata.Name) &&
                alchemyMetadata.Decimals > 0)
            {
                var tokenMetadata = new TokenMetadata(
                    contractAddress,
                    network,
                    alchemyMetadata.Symbol,
                    alchemyMetadata.Name,
                    alchemyMetadata.Decimals ?? 18,
                    alchemyMetadata.Logo,
                    "Alchemy");

                await _tokenMetadataRepository.AddOrUpdateAsync(
                    tokenMetadata,
                    updateExisting: true,
                    cancellationToken);

                _logger.LogInformation(
                    "Saved token metadata to cache/DB: {Symbol} ({Address}) on {Network}",
                    alchemyMetadata.Symbol,
                    contractAddress,
                    network);
            }
        ...
        {
            _logger.LogError(ex, "Error fetching token metadata from Alchemy for {Contract}", contractAddress);
            throw;
        }
    }
